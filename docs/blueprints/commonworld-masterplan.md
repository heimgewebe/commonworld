...
Die revisionsgebundene, schema-realistische Skalierungsprobe vom 2. August 2026 macht 1.000 und 10.000 Einträge zu den verbindlichen Vor-Cutover-Stufen. Bereits der vollständige 1.000er-Weltindex überschreitet mit 100.622 Byte gzip das 32.768-Byte-Startbudget; bei 10.000 Einträgen erreicht er 996.881 Byte gzip. Beide Vollindizes bleiben deshalb Export- und Prüfartefakte, während die größten bedarfsgeladenen 2-Hex-Shards mit 1.782 beziehungsweise 6.331 Byte gzip deutlich unter der Warnschwelle bleiben. Die Fixtures werden aus dem aktuellen CommonProject-Katalog abgeleitet, erneut gegen Schema v4 geprüft und binden Ortsmodi, Präsenzformen, Relationen, Provenienz, deutsche und englische Texte sowie inhaltsadressierte Details.

Der kompakte 100.000-Einträge-Stresstest erreicht 9.821.406 Byte gzip für den Vollindex und 39.205 Byte gzip für den größten von 256 2-Hex-Shards. Damit überschreitet er das harte Maximum von 32.768 Byte und hält den aktuellen Runtimepfad fail-closed. Eine 3-Hex-Neupartitionierung senkt den größten Shard auf 4.937 Byte, erzeugt in der flachen Vertragsform jedoch 212.467 Byte gzip Manifest und 293.345 Byte gzip Aggregat. Vor diesem Wachstum ist deshalb ein versionierter hierarchischer Manifest- und Aggregatvertrag samt Kompatibilitäts-, Recovery- und Browserbelegen umzusetzen; daraus folgt weder 100.000er-Produktionsreife noch eine Backend-, PostGIS- oder PMTiles-Pflicht ohne gemessenen statischen Engpass. Der ausführbare Vor-Cutover-Vertrag liegt in `contracts/commonworld/catalog-scale-gates.contract.json`.
...
Die kanonischen `CommonProject`-Dateien bleiben die einzige veränderbare Katalogwahrheit. Build und CI erzeugen daraus einen kompakten buildgebundenen Bootstrap für Karte, Suche, Filter, Fokus, Handlungswege, Quellenlinks und Prüfdaten. Redaktionelle Tiefenfelder, interne Quellen-IDs und Handoff-Metadaten bleiben ausschließlich in den vollständigen Projektdateien. Der Browser erzeugt die interaktiven Karten aus dieser Projektion; eine begrenzte statische No-JavaScript-Einstiegsseite bleibt genau einmal im HTML, statische interaktive Karten werden nicht zusätzlich dupliziert. Die begrenzte lineare Einstiegsseite liegt außerhalb der Bootstrap-Modulabhängigkeit und wird erst nach erfolgreichem interaktivem Start aus dem DOM entfernt; fällt das Bootstrap-Modul aus, bleibt sie als Recovery-Oberfläche sichtbar. Vollständiger DE/EN-Zugriff erfolgt über deterministisch paginierte Indizes mit höchstens 24 Identitäten je Seite und eine kanonische statische Projektseite je `CommonProject.id`; dadurch wächst die Landing-Page nicht mit dem vollständigen Katalog. Der Browser lädt die einzelnen Projektdateien beim Start nicht nochmals zur Gleichheitsprüfung; Manifest und vollständige Projekt-JSON bleiben dennoch öffentlich und direkt adressierbar.

Der aktuelle Entwurf wird nicht über eine feste Katalogzahl begrenzt. Der Vertrag `contracts/commonworld/catalog-delivery-budget.contract.json` misst rohe und komprimierte Bytes, doppelte Identitätslasten, Startanfragen, DOM-Knoten und vierfach CPU-gedrosselte Browserarbeit. Ab der Warnschwelle muss die Projektion neu gemessen werden; die harte Grenze darf nicht durch bloßes Hochsetzen des Budgets umgangen werden. Eine deterministische Budgetüberschreitung oder zwei aufeinanderfolgende Browserüberschreitungen erzwingen den erneuten Vergleich von kompakter Projektion, HTML-Hydration, segmentiertem statischem JSON und bedarfsgeladener schreibgeschützter Lieferung. Barrierefreiheit, SEO, No-JavaScript-Verhalten, Datenschutz, Caching, Komplexität und GitHub-Pages-Tauglichkeit bleiben gemeinsame Entscheidungskriterien.
