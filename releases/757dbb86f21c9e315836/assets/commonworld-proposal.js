...
const SENSITIVE_CONTEXT_PATTERN = /\b(?:latitude|longitude|coordinates?|gps|router|roof|dach|wohnung|apartment|household|haushalt)\b/iu;
...
const DMS_COORDINATE_PATTERN = /\d{1,3}\s*°\s*\d{1,2}\s*[′']\s*\d{1,2}(?:[.,]\d+)?\s*(?:[″"]|[′']{2})\s*[NS]\s+\d{1,3}\s*°\s*\d{1,2}\s*[′']\s*\d{1,2}(?:[.,]\d+)?\s*(?:[″"]|[′']{2})\s*[EW]/iu;
...
const STREET_WORD = String.raw`(?:street|road|avenue|boulevard|lane|drive|way|rue|calle|via|viale|corso|ulica|prospekt|st\.?|rd\.?|ave\.?|blvd\.?|ln\.?|dr\.?)`;
