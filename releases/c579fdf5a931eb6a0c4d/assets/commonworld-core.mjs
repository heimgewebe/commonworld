...
function validPublicExtentBbox(geometry) {
  ...
  const longitudeInside = (longitude) => west <= east
  ...
  return positions.length > 0 && positions.every(([longitude, latitude]) => (
    longitudeInside(longitude) && latitude >= south && latitude <= north
  ));
}
...
export function publicProjectNavigationTarget(publicMapData, projectId) {
  ...
  if (distinctExplicitBounds.length === 1) {
    const bounds = distinctExplicitBounds[0];
    const [[west, south], [east, north]] = bounds;
    const longitudeInside = (longitude) => west <= east
      ? longitude >= west && longitude <= east
      : longitude >= west || longitude <= east;
    if (coordinates.every(([longitude, latitude]) => longitudeInside(longitude) && latitude >= south && latitude <= north)) {
      return Object.freeze({
        kind: 'bounds',
