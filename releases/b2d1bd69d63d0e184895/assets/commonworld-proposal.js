import { CANDIDATE_LOCALES, normalizeKnownLocale } from './commonworld-locale-registry.mjs?v=6658799570ff';
...
if (CANDIDATE_LOCALES.includes(ACTIVE_LOCALE)) {
  const { WAVE1_LOCALE_PACKS } = await import('./commonworld-wave1-locales.mjs?v=d95f9e6ac5d1');
...
function validateCommonsBasisDraft(errors, draft, sources) {
  ...
  for (const [key, dimension] of entries) {
    ...
    if (!classificationIsValid) {
      ...
      errors.push(`${label}: ${hasPartialInput
        ? tr("Belegt, Unklar oder Nicht zutreffend wählen, weil Text oder Quellenverweise eingetragen wurden.", "choose Confirmed, Unknown, or Not applicable because text or source references were entered.")
