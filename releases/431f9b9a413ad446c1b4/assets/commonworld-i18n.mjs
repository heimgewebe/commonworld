...
} from './commonworld-locale-registry.mjs?v=20ad6de1cfac';
...
export async function loadWave1LocalePacks({
  importer = () => import('./commonworld-wave1-locales.mjs?v=d9db353b5711'),
...
export function localizeCatalogRecords(records, locale = DEFAULT_LOCALE) {
    ...
    const localized = sourceRecords.map((record) => ({
      ...
      provenance: record.provenance ? {
        ...
        sources: Array.isArray(record.provenance.sources)
          ? record.provenance.sources.map((source) => ({ ...source, _label_locale: 'de' }))
  ...
  const localized = sourceRecords.map((record) => {
    ...
    return {
      ...
      title: translation.title ?? englishTranslation.title ?? record.title,
