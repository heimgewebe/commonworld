import { FALLBACK_LOCALE, documentDirection, normalizeLocale, taxonomyLabel, text as i18nText } from './commonworld-i18n.mjs?v=211786268508';
...
export function mapTileCacheZoomLevels(deviceMemory) {
  const memoryGb = Number(deviceMemory);
  if (!Number.isFinite(memoryGb) || memoryGb <= 0) return 3;
  return memoryGb <= 4 ? 2 : 5;
}
...
export const DIGITAL_TAXONOMY = deepFreeze({
  ...
  nodes: [
    ...
    { id: 'mutual_local_self_help', parent_id: 'cooperation_self_organization', type: 'network', label_de: 'Nachbarschaft und gegenseitige Hilfe', order: 2, themes: ['care', 'mutual-aid', 'neighbourhood'] },
  ],
  compound_rules: [
  ...
  tie_rules: [
    ...
    { id: 'community_owned_energy_governance', candidate_node_ids: ['cooperative_governance', 'energy_cooperatives'], target_node_id: 'energy_cooperatives', reason: 'when community-owned energy and cooperative governance score equally, the provision-specific energy cooperative bundle is the narrower public path' },
  ],
...
function geometryCoordinatePairs(type, coordinates) {
  if (!validGeometryCoordinates(type, coordinates)) return [];
  if (type === 'Point') return [coordinates];
  if (type === 'Polygon') return coordinates.flat();
  return coordinates.flat(2);
}

function longitudeInsideNavigationBounds(longitude, west, east) {
  return west <= east
    ? longitude >= west && longitude <= east
    : longitude >= west || longitude <= east;
}

function validPublicExtentBbox(geometry) {
  ...
  const positions = geometryCoordinatePairs(geometry.type, geometry.coordinates);
  return positions.length > 0 && positions.every(([longitude, latitude]) => (
    longitudeInsideNavigationBounds(longitude, west, east) && latitude >= south && latitude <= north
  ));
}
...
function publicGeometry(location, representationKind) {
  ...
  if (validPublicExtentBbox(location.geometry)) geometry.bbox = [...location.geometry.bbox];
  return geometry;
}
...
function combinedExplicitNavigationBounds(explicitBounds, coordinates) {
  if (explicitBounds.length === 0) return null;
  const coordinateIsCovered = ([longitude, latitude]) => explicitBounds.some(([[west, south], [east, north]]) => (
    longitudeInsideNavigationBounds(longitude, west, east)
    && latitude >= south
    && latitude <= north
  ));
  if (!coordinates.every(coordinateIsCovered)) return null;

  const segments = [];
  let south = Infinity;
  let north = -Infinity;
  for (const [[west, boundSouth], [east, boundNorth]] of explicitBounds) {
    south = Math.min(south, boundSouth);
    north = Math.max(north, boundNorth);
  ...
  let largestGap = -1;
  let coverStart = merged[0][0];
  for (let index = 0; index < merged.length; index += 1) {
    const gapStart = merged[index][1];
    const nextStart = index + 1 < merged.length ? merged[index + 1][0] : merged[0][0] + 360;
    const gap = nextStart - gapStart;
    if (gap > largestGap) {
      largestGap = gap;
      coverStart = nextStart % 360;
    }
  }
  const span = 360 - largestGap;
  if (span >= 360) return [[-180, south], [180, north]];
  const west = coverStart > 180 ? coverStart - 360 : coverStart;
...
export function publicProjectNavigationTarget(publicMapData, projectId) {
  ...
  for (const feature of features) {
    ...
    if (validPublicExtentBbox(feature?.geometry)) {
      const [west, south, east, north] = feature.geometry.bbox;
      explicitBounds.push([[west, south], [east, north]]);
    }
  }
  if (coordinates.length === 0) return null;
  const distinctExplicitBounds = explicitBounds.filter((bounds, index) => explicitBounds.findIndex((candidate) => (
    candidate[0][0] === bounds[0][0]
    && candidate[0][1] === bounds[0][1]
    && candidate[1][0] === bounds[1][0]
    && candidate[1][1] === bounds[1][1]
  )) === index);
  const combinedExplicitBounds = combinedExplicitNavigationBounds(distinctExplicitBounds, coordinates);
  if (combinedExplicitBounds) {
    return Object.freeze({
      kind: 'bounds',
      bounds: Object.freeze(combinedExplicitBounds.map((corner) => Object.freeze(corner))),
    });
  }
...
export function semanticLocationLine({
  ...
  if (selected) {
    const contentLocale = typeof selected._title_locale === 'string' && selected._title_locale.trim()
      ? selected._title_locale
      : null;
    return {
      level,
