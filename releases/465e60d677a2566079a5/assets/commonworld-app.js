import { BOOTSTRAP_RECORDS } from './commonworld-bootstrap-catalog.mjs?v=76e426915037';
...
import { actionLabel, documentDirection, documentLocale, localizeCatalogRecords, text as i18nText, themeLabel, wave1LocalePackReady } from './commonworld-i18n.mjs?v=211786268508';
import {
  ...
  stateFromSearch,
  visibleDigitalNodes,
} from './commonworld-core.mjs?v=340abd90c91f';
...
function applyContentLanguage(element, locale, { originalText = false } = {}) {
  if (typeof locale === 'string' && locale.trim()) {
    element.setAttribute('lang', locale);
    element.setAttribute('dir', contentDirection(locale));
  } else {
    element.removeAttribute('lang');
    if (originalText) element.setAttribute('dir', 'auto');
    else element.removeAttribute('dir');
  }
}

function applyTitleLanguage(element, record) {
  applyContentLanguage(element, record?._title_locale ?? null, { originalText: true });
}
...
const COUNTRY_BOUNDARIES_URL = './assets/map/commonworld-country-boundaries.geojson?v=c6ee80ace6dc';
...
const RELEASE_CHECK_REQUEST_EVENT = 'commonworld:release-check-request';
...
const elements = {
  ...
  layerProjectStatus: document.querySelector('#layer-project-status'),
...
const runtime = {
  ...
  catalogDetailRecordsById: new Map(),
  ...
  releaseNavigationDeferred: false,
  ...
  mapRenderCount: 0,
  mapMoving: false,
  sphereRingPlaybackObserver: null,
  mapGeometryLastAt: 0,
...
function nonSerializedSpatialStateActive() {
  return Array.isArray(runtime.state.nearbyOrigin) && runtime.state.nearbyOrigin.length === 2;
}

function resumeDeferredReleaseCheck() {
  if (!runtime.releaseNavigationDeferred || nonSerializedSpatialStateActive()) return false;
  runtime.releaseNavigationDeferred = false;
  delete elements.stage.dataset.releaseNavigationDeferred;
  document.dispatchEvent(new Event(RELEASE_CHECK_REQUEST_EVENT));
  return true;
}

document.addEventListener(RELEASE_NAVIGATION_EVENT, (event) => {
  if (!nonSerializedSpatialStateActive()) return;
  event.preventDefault();
  runtime.releaseNavigationDeferred = true;
  elements.stage.dataset.releaseNavigationDeferred = 'spatial-filter';
});
...
function verifiedCatalogDetailRecord(identifier) {
  const cached = runtime.catalogDetailRecordsById.get(identifier);
  const generation = runtime.catalogPlatform?.manifest?.generation ?? null;
  return cached?.generation === generation ? cached.record : null;
}

function focusSourceLinks(record) {
  const detailRecord = verifiedCatalogDetailRecord(record?.id);
  if (!detailRecord) return record?.provenance?.sources ?? [];
  const localizedDetail = localizeCatalogRecords([detailRecord], LOCALE).records[0];
  return localizedDetail?.provenance?.sources ?? record?.provenance?.sources ?? [];
}
...
async function observeCatalogRecordShadow(identifier = runtime.state.project, { retrying = false } = {}) {
  ...
  if (!runtime.catalogShadowEnabled || !identifier) {
    runtime.catalogDetailRecordsById.clear();
    setCatalogRecordShadowState('idle');
    setCatalogDetailShadowState('idle');
    return;
  }
  // Only the current focus identity needs a full verified record. The bounded
  // load cache may retain transport work, but decoded canonical detail must not
  // accumulate as users browse a growing catalogue.
  runtime.catalogDetailRecordsById.clear();
  ...
  try {
    ...
    if (detailParity) {
      runtime.catalogDetailRecordsById.set(identifier, {
        generation: runtime.catalogPlatform.manifest.generation,
...
function installRecords(records) {
  ...
  runtime.digitalTreeCache = new WeakMap();
...
function treeForRecords(records = runtime.records) {
  ...
  const cached = runtime.digitalTreeCache.get(records);
  if (cached) return cached;
  const tree = buildDigitalPresentationTree(records, { locale: LOCALE });
  runtime.digitalTreeCache.set(records, tree);
  return tree;
}
...
function visibleDigitalView(records = visibleRecords()) {
  ...
  const cacheable = records === runtime.visibleRecordsCache?.records;
  const key = `${discoveryCacheKey()}\u001f${runtime.identityPresentationLimit}`;
  if (cacheable && runtime.visibleDigitalViewCache?.key === key) return runtime.visibleDigitalViewCache.view;
  const view = visibleDigitalNodes(treeForRecords(records), currentDigitalPath(), { identityLimit: runtime.identityPresentationLimit });
  if (cacheable) runtime.visibleDigitalViewCache = { key, view };
  return view;
}
...
function appendRingSequence(textPath, assignments, { prefix = '' } = {}) {
  ...
  for (const assignment of assignments) {
    ...
    applyContentLanguage(name, assignment.contentLocale, { originalText: !assignment.contentLocale });
...
function applySphereRingDetail(requestedDetailLevel = elements.sphere.dataset.ringDetailLevel || elements.sphere.dataset.detailLevel || 'names') {
  const detailLevel = compactSphereViewport.matches && requestedDetailLevel === 'names'
    ? 'compact'
...
function sphereRingOrbitPaused() {
  return elements.stage.dataset.mapMoving === 'true'
    || runtime.spherePointerStart !== null
    || elements.sphereEdge.matches(':focus');
}

function syncSphereRingOrbitPlayback() {
  const playbackRate = sphereRingOrbitPaused() ? 0 : 1;
...
function observeSphereRingOrbitPlayback() {
  if (runtime.sphereRingPlaybackObserver) return;
  runtime.sphereRingPlaybackObserver = new MutationObserver(syncSphereRingOrbitPlayback);
  runtime.sphereRingPlaybackObserver.observe(elements.stage, {
    attributes: true,
    attributeFilter: ['data-map-moving'],
  });
}

function renderSphereRibbons(records = runtime.records) {
  const view = visibleDigitalView(records);
  const visibleBundles = sphereRingBundleProjection(view, { nodeLimit: 8, childLimit: SPHERE_RING_BUNDLE_CHILD_LIMIT })
  ...
  const contentLocaleById = new Map(ringRecords.map((record) => [String(record.id), record._title_locale ?? null]));
  ...
  const bundleSummary = document.createElement('span');
  bundleSummary.textContent = t('visible_ring_bundles', 'Digitale Ringbündel: {bundles}.', { bundles: accessibleBundles.join('; ') });
  ...
  const nodes = [bundleSummary, document.createTextNode(' ')];
  if (accessibleAssignments.length) {
    ...
    nodes.push(intro, document.createTextNode(' '));
    const separator = DOCUMENT_DIRECTION === 'rtl' ? '، ' : ', ';
    accessibleAssignments.forEach((assignment, index) => {
      const title = document.createElement('span');
      title.className = 'sphere-ring-accessible-title';
      title.textContent = assignment.fullText;
      applyContentLanguage(title, assignment.contentLocale, { originalText: !assignment.contentLocale });
      nodes.push(title, document.createTextNode(index + 1 < accessibleAssignments.length ? separator : '.'));
    });
  } else {
    const empty = document.createElement('span');
    empty.textContent = t('visible_ring_commons_empty', 'In den digitalen Ringen sind derzeit keine Commons sichtbar.');
    nodes.push(empty);
  }
  elements.sphereRingAccessibleSummary.replaceChildren(...nodes);
  applySphereRingDetail();
  syncSphereRingOrbitPlayback();
...
function createRibbonSegment(record) {
  ...
  applyTitleLanguage(name, record);
...
function recordVisibleInCurrentSelection(record) {
  return Boolean(record)
    && visibleRecordIds().has(record.id)
    && projectMatchesDigitalState(record.id);
}

function identityRecordsForView(view = visibleDigitalView(), { includeSelected = true } = {}) {
  if (!isIdentityLevelView(view)) return [];
  const records = view.children.map((node) => runtime.recordsById.get(node.projectId)).filter(Boolean);
  const selected = includeSelected ? selectedProjectRecord() : null;
  if (recordVisibleInCurrentSelection(selected) && !records.some(({ id }) => id === selected.id)) records.push(selected);
  return records;
}
...
function renderLayerDeck() {
  ...
  runtime.laneResizeObserver = null;
  ...
  const identityLevel = isIdentityLevelView(view);
  ...
  for (const [index, node] of laneNodes.entries()) {
    ...
    if (identityLevel) {
      focus.classList.add('is-label');
    } else {
      focus.type = 'button';
      focus.setAttribute('aria-pressed', 'false');
      focus.setAttribute('aria-expanded', String(serializeDigitalPath(currentDigitalPath()) === node.pathKey));
      if (serializeDigitalPath(currentDigitalPath()) === node.pathKey) focus.setAttribute('aria-current', 'page');
      focus.addEventListener('click', () => selectDigitalBundle(node.path));
    }
    ...
    if (identityLevel && records.length < node.identityCount) {
      ...
      const increment = Math.min(DIGITAL_IDENTITY_DOM_LIMIT, node.identityCount - records.length);
...
function visibleRecordIds() {
  visibleRecords();
  return runtime.visibleRecordsCache?.ids ?? new Set();
}
...
function renderSemanticBreadcrumb(line) {
  ...
  line.crumbs.forEach((crumb, index) => {
    ...
    if (index === line.contentCrumbIndex) {
      applyContentLanguage(crumbNode, line.contentLocale ?? null, { originalText: !line.contentLocale });
...
function updateSemanticLocationLine() {
  ...
  const semanticLevelIsContent = finalCrumbIndex === line.contentCrumbIndex;
  applyContentLanguage(
    elements.semanticLevel,
    semanticLevelIsContent ? (line.contentLocale ?? null) : null,
    { originalText: semanticLevelIsContent && !line.contentLocale },
  );
...
function renderPrimaryOverlayState(record = selectedProjectRecord()) {
  ...
  const focusVisible = Boolean(record) && runtime.activeOverlay === null && !usesInlineLayerProjectDetail(record);
  ...
  for (const record of runtime.records) {
    for (const location of publicGeographicLocations(record)) {
      ...
      const contentLocale = typeof record._content_locale === 'string' && record._content_locale.trim()
        ...
        : null;
      const titleLocale = typeof record._title_locale === 'string' && record._title_locale.trim()
        ? record._title_locale
        : null;
      ...
      destinations.push(Object.freeze({
        ...
        contextLocale: contextIsInterfaceText ? null : titleLocale,
        contextOriginalText: !contextIsInterfaceText && !titleLocale,
...
function renderSpatialDestinationResults() {
  ...
  elements.spatialDestinationResults.replaceChildren(...matches.map((entry, index) => {
    ...
    applyContentLanguage(context, entry.contextLocale, { originalText: entry.contextOriginalText === true });
...
function createDiscoveryResult(record, position) {
  ...
  applyTitleLanguage(title, record);
...
function updateSphereResultVisibility() {
  ...
  elements.layerDeck.querySelectorAll('.digital-lane[data-layer-id]').forEach((lane) => {
    ...
    const focus = lane.querySelector('button.digital-lane-focus');
    if (focus) focus.setAttribute('aria-pressed', String(selected));
  });
  window.requestAnimationFrame(updateLaneOverflow);
}
...
function replaceLinks(container, links) {
  ...
  for (const link of links) {
    ...
    const labelLocale = typeof link?._label_locale === 'string' && link._label_locale.trim()
      ? link._label_locale
      : null;
    applyContentLanguage(anchor, labelLocale, { originalText: !labelLocale });
    if (labelLocale) anchor.dataset.contentLanguage = labelLocale;
    else delete anchor.dataset.contentLanguage;
    item.append(anchor);
    container.append(item);
  }
}
...
function updateFocusPanel() {
  ...
  applyTitleLanguage(elements.focusTitle, record);
  applyContentLanguage(elements.focusSummary, record._content_locale);
  ...
  const retainedDuringFiltering = filteringActive && !usesInlineLayerProjectDetail(record);
  elements.focus.toggleAttribute('data-retained-during-filtering', retainedDuringFiltering);
  elements.focusSelectionStatus.hidden = !retainedDuringFiltering;
  elements.focusSelectionStatus.textContent = retainedDuringFiltering
    ? t('selection_retained_during_filtering', 'Diese Auswahl bleibt geöffnet, während Suche oder Filter die übrige Ansicht verändern.')
    : '';
  ...
  replaceLinks(elements.focusSources, focusSourceLinks(record));
...
function focusVisibleHierarchyControl(pathKey) {
  ...
  const globeSelectors = [
    `.digital-lane[data-digital-path="${CSS.escape(pathKey)}"] button.digital-lane-focus`,
...
function updateSphereGeometry({ publishDiagnostics = true } = {}) {
  ...
  const ringDetailLevel = compactSphereViewport.matches && detailLevel === 'names' ? 'compact' : detailLevel;
...
function startCameraFlight(camera, { instant = false, duration = 260, onSettled = () => {} } = {}) {
  ...
  elements.stage.dataset.cameraFlightSettlement = 'pending';
  const map = runtime.map;
  if (!map) {
    elements.stage.dataset.cameraFlightSettlement = 'map-unavailable';
  ...
  let applyingFallbackTarget = false;
  const mapIsMoving = () => typeof map.isMoving === 'function' && map.isMoving();
  const complete = (settlement) => {
    if (completed || generation !== runtime.cameraFlightGeneration) return false;
    // A moveend listener can race MapLibre's final moving state. Never let a
    // timer or an early event replace the projected camera with side layout.
    if (mapIsMoving()) return false;
    completed = true;
    elements.stage.dataset.cameraFlightSettlement = settlement;
    runtime.cameraFlightCleanup?.();
    runtime.cameraFlightCleanup = null;
    onSettled();
    return true;
  };
  const settleAtFallbackTarget = () => {
    if (completed || generation !== runtime.cameraFlightGeneration || mapIsMoving()) return false;
    applyingFallbackTarget = true;
    applyCamera(camera, { instant: true, duration: 0 });
    applyingFallbackTarget = false;
    return complete('fallback-stop');
  };
  const onMoveEnd = () => {
    if (fallbackActive) {
      if (!applyingFallbackTarget) settleAtFallbackTarget();
      return;
    }
    complete('moveend');
  };
  // A missing moveend must not advance into side layout while MapLibre is still
  // moving. Bound the intervention, stop the camera, reconcile the exact target,
  // and only settle after an observed idle state. If idle cannot be confirmed,
  // retain the safe flight phase until MapLibre later reports completion.
  fallbackTimer = window.setTimeout(() => {
    if (complete('fallback-idle')) return;
    fallbackActive = true;
    elements.stage.dataset.cameraFlightSettlement = 'fallback-stopping';
    map.stop?.();
    const stopSettleDeadline = performance.now() + 240;
    const settleAfterStop = () => {
      if (settleAtFallbackTarget()) return;
      if (completed || generation !== runtime.cameraFlightGeneration) return;
      if (performance.now() >= stopSettleDeadline) {
        elements.stage.dataset.cameraFlightSettlement = 'fallback-awaiting-idle';
        return;
      }
      stopSettleTimer = window.setTimeout(settleAfterStop, 16);
    };
    settleAfterStop();
  ...
  // MapLibre easeTo synchronously stops a previous ease and emits its moveend.
  // Subscribe only after issuing this command so that stale event cannot settle
  // the replacement flight. An instant/synchronously completed command is
  // covered by the idle check immediately after subscription.
  applyCamera(camera, { instant, duration });
  map.on('moveend', onMoveEnd);
  if (!mapIsMoving()) complete('already-idle');
}
...
function wireControls() {
  observeSphereRingOrbitPlayback();
  ...
  elements.sphereEdge.addEventListener('blur', () => {
    elements.sphereFocus.style.display = 'none';
    syncSphereRingOrbitPlayback();
  });
  ...
  document.addEventListener('keydown', (event) => {
    ...
    else if (runtime.state.project) clearProject();
...
function createMap() {
  ...
  runtime.map = new window.maplibregl.Map({
    ...
    pixelRatio: Math.min(window.devicePixelRatio || 1, MAX_MAP_PIXEL_RATIO),
...
async function boot() {
  ...
  try {
    ...
    if (textSkipLink) {
      ...
      textSkipLink.setAttribute('href', `${target.pathname}${target.search}${target.hash}`);
    }
    ...
    // Only a selected identity's compact shard and generation-bound detail are read on demand;
    // discovery stays bootstrap-bound while verified full provenance upgrades the selected focus panel.
  } catch (error) {
    console.error(error);
