...
T016 is implemented as an isolated static proof under `proofs/search/`. It loads `examples/commonworld/search-index-input.sample.json` and filters the T014 allowed fields in the browser without introducing a search endpoint, search service, vector database, crawler, ingestion worker, submissions or write path. T017 adds transparent local match reasons and a browser-only proof score so search results can be inspected without treating ranking as authority. T018 anchors representative search queries in `examples/commonworld/search-query-fixtures.sample.json` so search quality drift is testable without runtime. T019 renders those static query fixtures in the search proof as read-only try-search buttons that only set local browser filters. T020 adds a proof-hub surface taxonomy so the entry page distinguishes visual explanation, location rendering, projection focus and static data-quality checks. T021 adds an offline hub smoke that checks the visible hub contract without starting a browser or server. T025 adds a static catalog snapshot to the proof hub so entry counts, curation-state counts and location-mode counts are visible before runtime and validated against `examples/commonworld/catalog-export.sample.json`. T026 renders static project preview cards from `examples/commonworld/search-index-input.sample.json`, keeping the visible project list tied to committed data without adding runtime fetching or write affordances. T027 adds a visual hierarchy pass so proof surfaces, catalog metrics, project previews and boundary notes are easier to scan while preserving the static-only contract. T028 adds `scripts/smoke_proof_hub_browser.py`, an operator-facing headless-browser screenshot smoke that writes desktop/mobile PNG captures under `.artifacts/` and emits a JSON receipt without committing screenshots. Run it manually with `make capture-proof-hub-browser-smoke`; it is intentionally not part of the default CI validation because local browser sandboxes differ. T029 adds `scripts/smoke_pages_live.py`, a read-only live Pages delivery smoke for operators; run it manually with `make smoke-pages-live` to detect DNS, HTTPS, redirect, parking-page or wrong-content drift without mutating GitHub Pages, DNS or INWX. T030 documents the exact INWX DNS repair for `commonworld.net` in `docs/ops/commonworld-pages-dns-fix.md` and adds `scripts/check_pages_dns_target.py`; run `make check-pages-dns-target` after the DNS change to verify that apex and `www` resolve to GitHub Pages.
