...
Englisch ist die öffentliche Standardsprache (`index.html`, `method.html`, `propose.html`). Deutsch bleibt als vollständig statische, öffentlich auswählbare Alternative über `de.html`, `method.de.html` und `propose.de.html` erreichbar. Der Sprachwechsel funktioniert daher auch ohne JavaScript.

Spanisch (`es`), Französisch (`fr`), brasilianisches Portugiesisch (`pt-BR`) und Arabisch (`ar`) werden als vollständige technische Kandidaten für Globus/Textansicht, Methodenseite und Vorschlagsseite erzeugt. Diese Vorschauen sind `noindex`, tragen einen sichtbaren Kandidatenhinweis und fehlen absichtlich in automatischer Erkennung und Sprachwahl. Aktivierung setzt eine unabhängige Sprachprüfung sowie Barrierefreiheits- und Browserbelege voraus. Arabisch nutzt zusätzlich einen expliziten RTL- und Mixed-Script-Vertrag. Die verbindliche Registry und Freigabelogik stehen in `docs/architecture/locale-release.contract.json`; die Kandidatentexte in `assets/locales/wave1-candidates.json`. Technische Kandidatenbelege unter `docs/evidence/locale-candidates/` sind ausdrücklich keine Freigabebelege.

Die fachliche Katalogwahrheit bleibt ausschließlich in `catalog/projects/*.json`. Präsentationsübersetzungen dürfen nur sichtbare Texte ersetzen; IDs, URLs, Geometrien, Quellen, Aktivitäts- und Kurationszustände bleiben kanonisch. Interface- und Inhaltssprache sind unabhängig: Wave-1-Kandidatenseiten kennzeichnen aktuell kanonische englische Katalogtexte ausdrücklich mit `lang="en"`, statt sie still als übersetzt auszugeben. `npm run build` erzeugt Registry, Sprachmodule, statische Oberflächen und Release-Snapshot deterministisch neu. Die Suche indexiert in der englischen Oberfläche zusätzlich die deutschen kanonischen Präsentationstexte, damit englische und deutsche Suchbegriffe parallel nutzbar bleiben.
