...
SHELL_REPLACEMENTS_EN = {
    ...
    '<legend>Erforderliche Präsenz</legend>': '<legend>Required presence</legend>',
    '> Vor Ort</label>': '> On site</label>',
    '> Digital</label>': '> Digital</label>',
    'Mehrere ausgewählte Merkmale müssen gemeinsam erfüllt sein (UND).': 'Multiple selected characteristics must all be present (AND).',
    '<h3>Ortsbezug</h3>': '<h3>Location scope</h3>',
    ...
    'Land und Umkreis schließen sich aus. Ein Umkreis wird erst nach Auswahl eines Orts oder Standorts aktiv. Die Ortssuche umfasst veröffentlichte Commonworld-Orte und Länder.': 'Country and radius are mutually exclusive. A radius becomes active only after choosing a place or location. Search covers published Commonworld places and countries.',
    '<summary>Weitere Filter</summary>': '<summary>More filters</summary>',
    ...
    '<span>Sortieren nach</span>': '<span>Sort by</span>',
    '<option value="auto">Empfohlen</option>': '<option value="auto">Recommended</option>',
    '<option value="catalogued">Neu im Katalog</option>': '<option value="catalogued">New to the catalogue</option>',
    '<option value="reviewed">Zuletzt geprüft</option>': '<option value="reviewed">Recently reviewed</option>',
    '<option value="title">Name A–Z</option>': '<option value="title">Name A–Z</option>',
    'Empfohlen passt sich dem Kontext an: Suche nach Passung, Umkreis nach Nähe, sonst bleibt die gefilterte Reihenfolge erhalten.': 'Recommended adapts to context: search by relevance, radius by proximity, otherwise the filtered order is retained.',
...
METHOD_REPLACEMENTS_EN = {
    ...
    ', ein unveränderliches Datum der ersten Katalogaufnahme, Quellen, Abrufdaten, Aktivitäts- und Kurationsangaben. Die JSON-Dateien sind dieselbe Datenwahrheit wie Globus und Textansicht. Fehlende Katalogeinträge bedeuten nicht, dass in einer Region keine Commons existieren.': ', an immutable date of first catalogue admission, sources, retrieval dates, activity information and curation information. The JSON files are the same data truth used by the globe and text view. Missing catalog entries do not mean that no Commons exist in a region.',
    '<h2>Was als Common gilt</h2>': '<h2>What counts as a Common</h2>',
