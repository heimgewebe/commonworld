...
def presentation_label(record: dict, locale: str = FALLBACK_LOCALE, public_geo_locations: list[dict] | None = None) -> str:
    ...
    if derivation:
        nodes = {node["id"]: node for node in TAXONOMY.get("nodes", [])}
        labels = [
            taxonomy_label(node_id, nodes[node_id]["label_de"], locale, ROOT)
            for node_id in derivation["path"]
            if node_id in nodes and node_id != TAXONOMY.get("root_id")
        ]
    ...
    *,
    ...
    for record in records:
        ...
        action = (
            ...
            f'data-commonproject-id="{identifier}" aria-pressed="false">{"Open" if normalize_locale(locale) == "en" else "Öffnen"}</button>\n'
            if interactive
            else ""
        )
        cards.append(
...
def render_locale_module(locale: str = DEFAULT_LOCALE, root: Path = ROOT) -> str:
    payload = json.dumps(load_locale(locale, root), ensure_ascii=False, separators=(",", ":"))
    payload = payload.replace("\u2028", "\\u2028").replace("\u2029", "\\u2029")
    return (
        f"// Generated from catalog/locales/{normalize_locale(locale)}.json. Do not edit.\n"
        f"export const COMMONWORLD_{normalize_locale(locale).upper()}_LOCALE = Object.freeze({payload});\n"
    )


def render_bootstrap_catalog(records: list[dict]) -> str:
    payload = json.dumps([bootstrap_record(record) for record in records], ensure_ascii=False, separators=(",", ":"))
    payload = payload.replace("\u2028", "\\u2028").replace("\u2029", "\\u2029")
    return (
        "// Generated by scripts/render_public_shell.py from the canonical catalog. Complete editorial detail remains available in public project JSON. Do not edit.\n"
...
<html lang="de">
  ...
  <body class="method-page">
    <main>
      ...
      <section><h2>Was als Common gilt</h2><p>Commonworld versteht ein Common als eine benennbare Ressource, Infrastruktur, Praxis oder Wissensbasis, die von mehreren Menschen gemeinsam genutzt, erhalten oder weiterentwickelt wird. Für die Aufnahme müssen gemeinsame Ressource, Gemeinschaft, Commoning-Praxis, Regeln und Verantwortung sowie gemeinsamer Nutzen durch primärnahe Quellen belegt sein. Open Source, Kostenfreiheit, Gemeinnützigkeit, Dezentralität oder viele Nutzer reichen für sich allein nicht aus. Die Formen bleiben offen; die Begründung ist verbindlich und im <a href="./contracts/commonworld/commons-definition.contract.json">Commons-Definitionsvertrag</a> maschinenlesbar.</p></section>
