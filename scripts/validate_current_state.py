...
def validate_current_state(root: Path = ROOT) -> list[str]:
    ...
    cache_limits = catalog_delivery.get("runtime_catalogue_cache_limits", {})
    if (
        runtime_cache.get("shards_max_entries") != cache_limits.get("shards")
        or runtime_cache.get("details_max_entries") != cache_limits.get("details")
    ):
